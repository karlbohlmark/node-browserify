...
Wrap.prototype.addEntry = function (file, opts) {
    ...
    required.strings.forEach(function (r) {
        var x = r.match(/^(\.\.?)?\//) ? path.resolve( path.join( opts.root, path.relative(opts.root,dir) ), r) : r;
        self.require(x, { root : opts.root });
    });
    
    this.entries[file] = this.appends.length;
    
    var target = opts.target || file.slice(opts.root.length);
...
function makeTarget (file, root) {
    ...
    else if (s === nm && /^\/[^\/]+-browserify/.test(s)
    && file.slice(s.length), /^[^\/]+-browserify/.test(file.slice(s.length))) {
